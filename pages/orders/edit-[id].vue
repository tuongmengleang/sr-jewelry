...
/* states */
const { $humanPrice, $storageFile, $getPagination } = useNuxtApp()
const client = useSupabaseClient()
const route = useRoute()
const router = useRouter()
const activeProducts: any = ref<Array<string>>([])
const uiStore = useUI()
const productStore: any = useProduct()
...
/* Fetch Data */
const { data: customers, refresh }: any = await useAsyncData('customers', () =>
  $fetch('/api/customers', {
    method: 'GET',
    params: {
      page: 0,
...
const { data, error } = await useAsyncData(
  ...
  async (): Promise<any> => {
    const { data, error } = await client
      .from('orders')
      .select(
        'id,  customers(*), order_items(id, order_id, quantity, price, products(*, product_images(image_path)))'
      )
  ...
  {
    transform: (resp: any) => {
      selectedCustomer.value = resp.data.customers
      resp.data.order_items.forEach((item: any) => {
        totalPrice.value += item.price
        activeProducts.value.push(item.products)
      })
    },
  }
)
...
const onChangeFilter = debounce((event: any): void => {
...
const onPurchase = async (): Promise<void> => {
  ...
  useToast().success('កែប្រែបញ្ជាទិញដោយជោគជ័យ')
}
      ...
      <ClientOnly>
        <div class="grid grid-cols-12 gap-5">
          ...
          >
            ...
            <div class="space-y-3 mt-7">
              ...
              <div
                ...
                class="w-full pt-5"
              >
                <Button :loading="uiStore.isLoading" @click="onPurchase">
                  បញ្ជាទិញ
                </Button>
              </div>
            </div>
          ...
          <Modal v-model="active" :outside-close="true" size="2xl">
            ...
            <div
              class="py-4 max-h-[70vh] overflow-y-auto custom-scrollbar-thin"
            >
              <div class="w-full flex items-center gap-3">
                ...
                <Popover v-slot="{ open }" class="relative">
                  <PopoverButton
                    :class="
                      open ? 'border-gray-300 bg-gray-50' : 'text-opacity-90'
                    "
                        ...
                        <div class="p-4">
                          ...
                          <div class="space-y-3 pt-3">
                            ...
                            <Disclosure v-slot="{ open }">
                              ...
                              >
                                <div
                                  v-for="c in productStore.gemstones"
                                  :key="c.value"
                                  class="relative flex items-center gap-3 px-2 py-1 shadow border border-gray-200 rounded-lg hover:border-gray-300 cursor-pointer"
                                  :class="{
                                    'bg-gray-100 text-gray-700 font-semibold':
